...
#     * DEPLOY_VE_PVE_HOST
#     * DEPLOY_VE_DEBIAN_ID # <- VE ID
#     * DEPLOY_VE_DEBIAN_IP # <- Ex.: 192.168.0.20/24
...
deploy_ve_config() {
  ...
  CREATE_FLAGS=(              # <- Create LXC flags, '--password' is autoappended with value from EXTRAS[root_pass]
    ...
    --net0="name=eth0,bridge=vmbr0,ip=${DEPLOY_VE_DEBIAN_IP},gw=${DEPLOY_VE_GW}"
    --storage="${DEPLOY_VE_STORAGE}"
    --hostname=ansible-debian.test.home
    --rootfs=20
    --timezone=host
    --onboot=0
  ...
  create_lxc_user() {
    ...
    log_info "${FUNCNAME[0]} (${name}) ..."
      cat <<< "${name}:${pass}" | lxc-attach -n "${VE_ID}" -- /bin/sh -c "
        set -x
        groupadd -g '${uid}' '${name}' \
        && useradd -g '${uid}' -G sudo -u '${uid}' -m '${name}' \
        && chpasswd -e
      " || return
